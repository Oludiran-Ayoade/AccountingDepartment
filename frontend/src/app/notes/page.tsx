  ...
  return (
    <div className="min-h-screen py-12 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-background via-background to-primary/5">
      <div className="max-w-7xl mx-auto">
        ...
        {selectedCourse && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-2 sm:p-4 animate-in fade-in duration-300" onClick={() => setSelectedCourse(null)}>
            <div className="bg-background rounded-2xl sm:rounded-3xl max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-hidden shadow-2xl border border-border/50 animate-in zoom-in-95 duration-300" onClick={(e) => e.stopPropagation()}>
              <div className="p-3 sm:p-6 border-b border-border/50 flex items-start justify-between bg-gradient-to-r from-primary/5 to-transparent">
                <div className="flex-1 min-w-0 pr-2">
                  <div className="flex flex-wrap items-center gap-2 mb-2 sm:mb-3">
                    <span className="px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-gradient-to-r from-primary/20 to-primary/10 text-primary text-xs sm:text-sm font-bold uppercase tracking-wide shadow-sm">
                      {selectedCourse.code}
                    </span>
                    <span className="px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg bg-gradient-to-r from-accent/80 to-accent/60 text-foreground text-xs sm:text-sm font-semibold shadow-sm">
                      {selectedCourse.level} Level - {selectedCourse.semester === 'first' ? 'First' : 'Second'} Semester
                    </span>
                  </div>
                  <h2 className="text-xl sm:text-2xl lg:text-3xl font-extrabold text-foreground line-clamp-2">{selectedCourse.name}</h2>
                </div>
                <button
                  onClick={() => setSelectedCourse(null)}
                  className="p-1.5 sm:p-2.5 hover:bg-red-500/10 rounded-xl transition-all duration-300 hover:scale-110 group flex-shrink-0"
                >
                  <X className="w-5 h-5 sm:w-6 sm:h-6 text-muted-foreground group-hover:text-red-500 transition-colors" />
                </button>
              </div>
              ...
              <div className="p-3 sm:p-6 overflow-y-auto max-h-[calc(95vh-100px)] sm:max-h-[calc(90vh-120px)]">
                {isLoadingNotes ? (
                  ...
                  <div className="text-center py-8 sm:py-16">
                    <div className="w-16 h-16 sm:w-24 sm:h-24 mx-auto mb-4 sm:mb-6 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center">
                      <FileText className="w-8 h-8 sm:w-12 sm:h-12 text-primary" />
                    </div>
                    <h3 className="text-xl sm:text-2xl font-bold text-foreground mb-2">No notes available</h3>
                    <p className="text-muted-foreground text-base sm:text-lg">No notes have been uploaded for this course yet.</p>
                  </div>
                ) : (
                  <div className="space-y-3 sm:space-y-5">
                    {courseNotes.map((note) => (
                      <div key={note.id} className="group bg-gradient-to-br from-background to-background/80 rounded-xl sm:rounded-2xl shadow-md hover:shadow-xl border border-border/50 hover:border-primary/30 transition-all duration-300 overflow-hidden">
                        <div className="p-3 sm:p-5">
                          <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
                            <div className="flex-1 w-full">
                              <h3 className="text-base sm:text-xl font-bold text-foreground mb-1 sm:mb-2 group-hover:text-primary transition-colors">{note.title}</h3>
                              <p className="text-xs sm:text-sm text-muted-foreground mb-3 sm:mb-4 leading-relaxed line-clamp-2">{note.description}</p>
                              <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs sm:text-sm text-muted-foreground">
                                ...
                                <span className="px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-lg bg-gradient-to-r from-primary/10 to-primary/5 text-primary text-xs font-semibold">{note.fileType || 'PDF'}</span>
                              </div>
                            </div>
                            <div className="flex sm:flex-col gap-2 sm:gap-3 w-full sm:w-auto">
                              <Button
                                variant="outline"
                                size="sm"
                                onClick={() => setViewingPdf({url: note.fileUrl, name: note.title})}
                                className="shadow-sm hover:shadow-md transition-shadow flex-1 sm:flex-none text-xs sm:text-sm"
                              >
                              ...
                              <Button
                                ...
                                className="shadow-md hover:shadow-lg transition-shadow flex-1 sm:flex-none text-xs sm:text-sm"
                              >
